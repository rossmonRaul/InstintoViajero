...
export const InputText = ({ id, label, type, placeholder, value, text, onChange, mensajeValidacion, className, onClick, readOnly, disabled, required = true, max }) => {
    return (
        <Form.Group className={"mb-3 " + className} controlId={id}>
            <Form.Label>{label}</Form.Label>
            <Form.Control type={type} placeholder={placeholder} size="sm" value={value} onChange={onChange} onClick={onClick} required={required} readOnly={readOnly} disabled={disabled} max={max}/>
